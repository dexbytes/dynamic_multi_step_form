...
///
/// Used by [DropdownButtonNew.selectedItemBuilder].
...
class _DropdownMenuPainter extends CustomPainter {
  _DropdownMenuPainter({
    ...
    required this.getSelectedItemOffset,this.dropdownDecoration,
...
class _DropdownMenuItemBottomButton<T> extends StatefulWidget {
  const _DropdownMenuItemBottomButton({
    Key? key,
    this.padding,
  ...
  @override
  __DropdownMenuItemBottomButtonState<T> createState() => __DropdownMenuItemBottomButtonState<T>();
}

class __DropdownMenuItemBottomButtonState<T> extends State<_DropdownMenuItemBottomButton<T>> {
  ...
  void _handleOnTap() {
    final DropdownMenuItemNew<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
    ...
    Widget child = Container(
      ...
      child: Row(children: [
        TextButton(onPressed: (){
          Navigator.pop(
            context);
        }, child: const Text("Cancel")),TextButton(onPressed: (){}, child: const Text("Ok"))
      ],),
    );
...
class _DropdownMenu<T> extends StatefulWidget {
  const _DropdownMenu({
    ...
    required this.enableFeedback,
    this.borderRadius,
    this.dropdownPadding,this.dropdownDecoration,
  }) : super(key: key);
    ...
    return FadeTransition(
      ...
      child: CustomPaint(
        painter: _DropdownMenuPainter(
          ...
          getSelectedItemOffset: () => route.getItemOffsetNew(route.selectedIndex,kMaterialListPadding.top),
        ),
        child: Semantics(
          scopesRoute: true,
          namesRoute: true,
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  double getItemOffsetNew(int index, double paddingTop) {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    ...
    final double selectedItemOffset = getItemOffsetNew(index,dropdownPadding != null
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // DropdownButtonNew.itemHeight is specified or DropdownButtonNew.itemHeight is null
...
// The container widget for a menu item created by a [DropdownButtonNew]. It
// provides the default configuration for [DropdownMenuItemNew]s, as well as a
// [DropdownButtonNew]'s hint and disabledHint widgets.
...
/// An item in a menu created by a [DropdownButtonNew].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class DropdownMenuItemNew<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownMenuItemNew({
    Key? key,
    this.onTap,
    this.value,
    this.enabled = true,
  ...
  ///
  /// Eventually returned in a call to [DropdownButtonNew.onChanged].
  final T? value;
...
/// An inherited widget that causes any descendant [DropdownButtonNew]
...
/// Typically, an enum is used. Each [DropdownMenuItemNew] in [items] must be
...
///
/// {@tool dartpad}
/// This sample shows a `DropdownButtonNew` with a large arrow icon,
...
class DropdownButtonNew<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [DropdownMenuItemNew] values. If [items] or
  ...
  DropdownButtonNew({
    ...
    this.dropdownColor,this.dropdownDecoration,
    this.menuMaxHeight,
    this.enableFeedback,
    this.alignment = AlignmentDirectional.centerStart,
    this.borderRadius,  this.offset,this.dropdownOverButton = false,
    // When adding new arguments, consider adding similar arguments to
    // DropdownButtonFormField.
  }) : assert(items == null || items.isEmpty || value == null ||
      items.where((DropdownMenuItemNew<T> item) {
        return item.value == value;
      }).length == 1,
  "There should be exactly one item with [DropdownButtonNew]'s value: "
      '$value. \n'
  ...
  final List<DropdownMenuItemNew<T>>? items;

  /// The value of the currently selected [DropdownMenuItemNew].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [DropdownButtonNew.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [DropdownButtonNew.disabledHint] widget if it is non-null.
  /// If [DropdownButtonNew.disabledHint] is also null but [DropdownButtonNew.hint] is
  /// non-null, [DropdownButtonNew.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [DropdownMenuItemNew]s in [items].
  ///
  /// When a [DropdownMenuItemNew] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropdownMenuItemNew] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `DropdownButtonNew` with a button with [Text] that
  /// corresponds to but is unique from [DropdownMenuItemNew].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [DropdownMenuItemNew] from [items]
  ...
  ///
  /// {@tool dartpad}
  /// This sample shows a `DropdownButtonNew` with a dropdown button text style
  ...
  @override
  State<DropdownButtonNew<T>> createState() => _DropdownButtonState<T>();
}

class _DropdownButtonState<T> extends State<DropdownButtonNew<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  @override
  void didUpdateWidget(DropdownButtonNew<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null
        ...
        || (widget.value == null &&
            widget.items!
                .where((DropdownMenuItemNew<T> item) => item.enabled && item.value == widget.value)
                .isEmpty)) {
      _selectedIndex = null;
      return;
    }

    assert(widget.items!.where((DropdownMenuItemNew<T> item) => item.value == widget.value).length == 1);
  ...
  Widget build(BuildContext context) {
    ...
    if (!DropdownButtonHideUnderlineNew.at(context)) {
      ...
      result = Container( decoration: widget.dropdownDecoration??BoxDecoration(
        ...
        padding: EdgeInsets.only(left: widget.dropdownDecoration!=null?5:0),
        child: Stack(
          children: <Widget>[
            result,
            widget.dropdownDecoration!=null?Container():Positioned(
              left: 0.0,
              right: 0.0,
              bottom: bottom,
...
/// A [FormField] that contains a [DropdownButtonNew].
///
/// This is a convenience widget that wraps a [DropdownButtonNew] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [DropdownButtonNew], which is the underlying text field without the [Form]
///    integration.
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [DropdownButtonNew] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  DropdownButtonFormField({
    Key? key,
    required List<DropdownMenuItemNew<T>>? items,
  ...
  "There should be exactly one item with [DropdownButtonNew]'s value: "
      ...
      'with the same value',
        ...
        builder: (FormFieldState<T> field) {
          ...
          final bool showSelectedItem = items != null && items.where((DropdownMenuItemNew<T> item) => item.value == state.value).isNotEmpty;
